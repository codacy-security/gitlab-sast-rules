...
def main()
  ...
  Dir.glob('mappings/*.yml').each do |mapping_file|
    ...
    rulesets = {
      License::MIT[:name] => {}, # root dir
      License::SEMGREP[:name] => {}
    }
    ...
    rule_file_paths.each do |rule_file_path|
      ...
      rule['id'] = newid
      license_name = rule['metadata']['license']
      ...
      rulesets[license_name][newid] = rule
      rulesets[license_name][newid]['metadata'].merge!('primary_identifier' => primary_id)
      rulesets[license_name][newid]['metadata'].merge!('secondary_identifiers' => secondary_ids)
    end

    rulesets.each do |license_name, rules|
      next unless rules.any?

      dir = File.join(dest, License.by_name(license_name)[:dir])
      Dir.mkdir(dir) unless Dir.exist?(dir)

      target_file = File.join(dir, "#{prefix}.yml")
      content = { 'rules' => rules.values }

      puts("writing #{rules.values.length} rules into #{target_file}")
      write_to_file(target_file, version, content)
    end

  end

  # copy license files into dist
  License.copy_license_files(dest)

end

def write_to_file(path, version, content)
  formatted = AutoFormat.reformat_yaml('', content)
